...
def test_map_to_pairs():
    maps = {"/foo": "/mnt/ext/foo", "/foo/bar": "/mnt/ext/foo/bar"}
    pairs = map_to_pairs(maps, False)
    assert pairs == [("/foo/bar", "/mnt/ext/foo/bar"), ("/foo", "/mnt/ext/foo")]

    pairs = map_to_pairs(maps, True)
    assert pairs == [("/mnt/ext/foo/bar", "/foo/bar"), ("/mnt/ext/foo", "/foo")]
